...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const whoami = useCallback(async () => {
    try {
      ...
      if (response instanceof Response && response.ok) {
        const data: { name: string; email: string; roles: string[] } =
          await response.json();
        setAuthState((prev) => ({
          ...prev,
  ...
  const logout = useCallback(async () => {
    try {
      setIsLoading(true);
      const response = await fetch(
        "http://localhost:3001/api/auth/refresh/logout",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
...
export const useAuthRedirect = () => {
  ...
  useEffect(() => {
    if (!isLoggedIn && !isLoading && !pathname.includes(roles.ADMIN)) {
      router.push("/login");
    } else if (!isLoggedIn && !isLoading && pathname.includes(roles.ADMIN)) {
      router.push("/admin/login");
    }

    //redirect user with user role to user dash
    if (
      isLoggedIn &&
      user &&
      user.role === roles.USER &&
      pathname.includes(roles.ADMIN)
    ) {
      router.push("/dashboard");
    }
  }, [isLoggedIn, isLoading, router, pathname, user]);
