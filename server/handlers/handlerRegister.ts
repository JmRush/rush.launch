...
import { BadRequestError, ForbiddenError, InternalServerError, UnauthorizedError } from "../types/types_error";
...
export const handlerRegister = async (req: Request, res: Response) => {
    try {
        ...
        const user = await transactionCreateUserRole(name, email, hashedPassword, roleId.id);
        if(!user) {
        ...
        //return success
        res.status(200).json({success: true, id: user.id, name: user.name, email: user.email, roles: ["user"]});
    } catch(error) {
